...
class OktaBase:
    def __init__(self):
        ...
        if self.OKTA_BASE_URL and self.OKTA_API_TOKEN is None:
            raise ValueError(
                "You should setup both `OKTA_BASE_URL` and `OKTA_API_TOKEN` in env vars"
            )
    ...
    def _api_call(self, request_type, url_action, payload=None, data=None, params=None):
        try:
            response = requests.request(
                request_type,
                f"{self.OKTA_BASE_URL}{url_action}",
...
class OktaService(OktaBase):
    def create_user(self, payload):
        try:
            response = self._api_call(
                request_type="POST", url_action="/users", payload=payload
            )
            return response
    ...
    def login_user(self, payload):
        try:
            response = self._api_call(
                request_type="POST", url_action="/authn", payload=payload
            )
            return response
        except Exception as e:
            return e
    ...
    def logout_user(self, payload):
        try:
            response = self._api_call(
                request_type="POST", url_action="/authn/cancel", payload=payload
            )
            return response
        except Exception as e:
            return e
