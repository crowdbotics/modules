  ...
  const renderItem = ({item}) => {
    return (
      <View style={styles.listItemContainer}>
            ...
            <Text><Text style={styles.bold}>Payment Method:</Text> {item.payment_method_types?.[0]} {item.charges?.data?.[0]?.payment_method_details?.card?.brand} - {item.charges?.data?.[0]?.payment_method_details?.card?.last4} </Text>
