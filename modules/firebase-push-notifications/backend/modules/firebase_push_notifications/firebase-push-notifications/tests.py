...
class NotificationTestCase(APITestCase):
    def setUp(self):
        ...
        self.user_token = Token.objects.create(user=self.user)
        self.user_token.save()
        self.receiver_user = User.objects.create(username="david", email="david661@semonir.com",
                                                 password="david123@")
        self.receiver_user_token = Token.objects.create(user=self.receiver_user)
        self.receiver_user_token.save()
        self.fcm_devices = FCMDevice.objects.create(name="test_device", device_id="112233",
                                                    registration_id="ePs6mLMc6ExsmZOPR7H9i5:APA91bGg5i8w2TwBQ7KsWr0e3Y4ykSB3TOI97I6H-vTVv_eChK2Ad2fcvRl3KfPz_2d8Y2nyZIbFpGU42LKEkYOVfmyd51_uTlEdC9kBrNOcHz7e3jmRTL4QAPh75I6wogFkmAq6_y1b",
                                                    type='ios')
    ...
    def test_get_notification_details(self):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.receiver_user_token.key)
        url = '/modules/firebase-push-notifications/notification/'
        image = BytesIO()
        Image.new('RGB', (100, 100)).save(image, 'JPEG')
        image.seek(0)
...
class UserNotificationTestCase(APITestCase):
    ...
    def test_create_user_notification(self):
        ...
        user_notification_url = '/modules/firebase-push-notifications/user-notification/'
        user_notification_data = {
            'notification': notification_response.data['id']
        }
        response = self.client.post(user_notification_url, user_notification_data, format='json')
...
class ModelStrTestCase(APITestCase):
    def setUp(self):
        ...
        image.seek(0)
        image_1 = SimpleUploadedFile('image.JPG', image.getvalue())
        self.create_notification = Notification.objects.create(sender=self.user, receiver=self.receiver_user,
                                                               title="message", message="hi this is demo",
                                                               image=image_1)
        self.user_notification = UserNotification.objects.create(user=self.user, notification=self.create_notification)
