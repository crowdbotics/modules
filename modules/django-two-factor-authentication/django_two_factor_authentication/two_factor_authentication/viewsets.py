...
class TwoFactorAuthViewSet(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        :param request: Contains an object which have field 'method' (email, phone_number, google_authenticator).
        google_authenticator will retuns the QR code link which you can you to register on Google Authenticator App.
        """
        try:
            data = request.data
            user = request.user
            ...
            if auth_serializer.is_valid():
                if validate_data["method"] == "email":
                    ...
                    return Response({'message': "Verification code has been sent to your Phone number"},
                                    status=status.HTTP_200_OK)
                elif validate_data["method"] == "google_authenticator":
                    name = user.email
            ...
            if verification_serializer.is_valid():
                if validate_data["method"] == "google_authenticator":
                    ...
                    if totp_code == validate_data["code"]:
                        ...
                        return Response({'message': 'Not Verified'}, status=status.HTTP_406_NOT_ACCEPTABLE)
                else:
