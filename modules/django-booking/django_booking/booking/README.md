...
| `/modules/booking/booking-details/` | object `{identity_number, type, description, occupancy, from_date, to_date, status, booking, penalty, plan}` | Will take the unique `identity_number` for the booking, `type`, `description` and `status` of the booking, id's of the selected plan, booking and penalty as `plan`, `booking` and `penalty` in `POST` request. |
